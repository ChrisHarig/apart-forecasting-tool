...
- Forecast benchmark services now create reproducible benchmark dataset snapshots before scoring. Built-in baselines, optional whitelisted StatsForecast AutoETS, disabled-by-default experimental TabPFN-Time-Series metadata, and uploaded prediction CSV sets are evaluated against the same stored aggregate train/test split, holdout dates, observed values, and scoring rules. Uploaded model support remains prediction CSV only; the backend does not execute uploaded model code.
