...
- Verified frontend commands pass:
  - `npm test` via bundled npm: 7 test files, 21 tests passed.
...
- If feasible, test map hover/click selection with MapLibre events. If not feasible, document the limitation and test the nearest stable contract.
...
Suggested scope:
- Test no-country-selected empty state.
- Test selected-country no-data empty state.
...
Goal: Improve hover panel behavior for future news/scraper integration without inventing news.

Suggested scope:
- Review current hover-country and news-summary flow.
- Add loading/error/empty display states for country news.
- Add tests for no-news-backend empty state.
- Make it clear the frontend does not scrape.
- Keep hover panel compact and map-focused.

Acceptance criteria:
- Hover panel shows country, source coverage count, and clean news empty state.
...
- Ensure fallback copy is clean and does not imply data loss beyond basemap unavailability.
